/** https://github.com/frzi/blinkjs */
(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports) :
        typeof define === 'function' && define.amd ? define(['exports'], factory) :
            (factory((global.blink = {})));
    ...
    const device = function () {
        ...
        const { debugRendererInfo } = extensions;
    ...
    class Texture {
        ...
        copy() {
            const copy = new Texture(this.internalFormat, this.width, this.height, this.format, this.type, null, this.alignment);
    ...
    const readablesMap = new WeakMap();
    const writablesMap = new WeakMap();

    class Buffer {
        constructor({ alloc, data, type = FLOAT, vector = 1, wrap = CLAMP, width = 0, height = 0 }) {
            ...
            const size = alloc || data.length;
            // https://github.com/frzi/blinkjs/issues/9
            this.dimensions = [width, height]; // closestDimensions(size / this.vector);
        ...
        _finish() {
            ...
            const writable = this._getWritable();
            if (writable) {
                writable.delete();
    ...
    class DeviceBuffer {
        constructor({ alloc, data, type = FLOAT, vector = 1, wrap = CLAMP }) {
            ...
            const texture = this._getReadable(true);
        ...
        copy() {
            const copyReadable = this._readable.copy();
            const copyBuffer = new DeviceBuffer({
                alloc: this.size,
                type: this.type,
                vector: this.vector
            });
        ...
        _finish() {
            ...
            if (writableCopy) {
                const readableCopy = this._getReadable();
                if (readableCopy) {
                    readableCopy.delete();
                }
...
void main() {
    ...
    class Program {
        constructor(fragSource) {
            ...
            /// UBO CREATION TAKEN FROM https://gist.github.com/jialiang/2880d4cc3364df117320e8cb324c2880
            // ==== START OF PART A ====

            // Get the index of the Uniform Block from any program
            const blockIndex = gl.getUniformBlockIndex(this.id, "data_ubo");
            ...
            gl.bufferData(gl.UNIFORM_BUFFER, blockSize, gl.STREAM_COPY /* gl.DYNAMIC_DRAW */);
            ...
            const index = gl.getUniformBlockIndex(this.id, "data_ubo");
            gl.uniformBlockBinding(this.id, index, 1);

            // ==== END OF PART B ====
        }

        delete() {
            gl.deleteBuffer(this.uboBuffer);
...
highp uint bl_Id() {
    ...
    class Kernel {
        constructor(io, source) {
            ...
            const inputCount = Object.values(this.inputs).length;
    ...
    function prepareFragmentShader(inputs, outputDescriptors, source) {
        const uniforms = Object.entries(inputs).map(([name, buffer]) => {
            const { inputType, precision } = buffer.formatInfo;
            return `uniform ${precision} ${inputType} ${name};`
        });

        const outs = Object.entries(outputDescriptors).map(([name, props]) => {
